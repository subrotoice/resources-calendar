const useActivitiesTypes = async () => {
  const res = await fetch(
    "https://dev-api.soms.gov.bd/gw/resource-service/api/activities/types",
    {
      method: "GET",
      headers: {
      ...
      cache: "no-store",
      next: { revalidate: 0 }, // Revalidate cache every 10 seconds
    }
  );
...
export default useActivitiesTypes;
